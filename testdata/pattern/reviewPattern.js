...
const review = abstractRepository('Review');
  ...
  constructor(){
    ...
    for(var i=1;i<=4;i++){
      const d = {
        ...
        event_name: "テストイベント",
        date_key: dt.setDate(dt.getDate() - (i-1))
      }
  ...
  genReviewData(){
    return new Promise((resolve, reject) => {
      ...
      .then(res => {
        ...
        // スケジュールデータ作成
        return schedule.bulkCreate(this.review_schedule_data)
      })
      .then(res => {
        // マッチングデータ作成
        return matching.bulkCreate(this.review_matching_data)
      })
      .then(res => {
        // レビューデータ作成
        return review.bulkCreate(this.review_data)
      })
      .then(res => {
