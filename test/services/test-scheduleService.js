    ...
    describe('○正常テスト', function () {
      it('指定した年月(2018/08)のスケジュールリストを取得する。', () =>  {
        ...
        const stub_return_obj = [
          { id: 1, user_id: 1, schedule_type: 2, group_year: 2018, group_month: 8, date_key: dateHelper.createDate(2018, 8, 1).toDate(), time_from: "11:00", time_to: "17:00", shot_type: 1, event_name: "テストイベント", event_url: "htt://c2link.com/detail/event/test", cost: 1000, num: 1, cos_chara: null, remarks: "カメラマンテストスケジュール" },
          { id: 2, user_id: 1, schedule_type: 2, group_year: 2018, group_month: 8, date_key: dateHelper.createDate(2018, 8, 16).toDate(), time_from: "11:00", time_to: "17:00", shot_type: 1, event_name: "テストイベント", event_url: "htt://c2link.com/detail/event/test", cost: 1000, num: 1, cos_chara: null, remarks: "カメラマンテストスケジュール" }
        ]
        stub.withArgs(1, 2018, 8).returns(stub_return_obj);
        ...
        .then(res => {
          ...
          res.forEach(ele => {
            ...
            // 1日と16日にスケジュールが存在する。
            if(ele.day === 1){
              expect(ele.schedule[0]).to.deep.equal(stub_return_obj[0])
            }
            else if(ele.day === 16){
              expect(ele.schedule[0]).to.deep.equal(stub_return_obj[1])
            }
          });
        })
      });
  ...
  describe('::getScheduleData()', function() {
    describe('○正常テスト', function () {
      it('指定したスケジュールIDのスケジュールを取得する。', () => {
        ...
        const stub_return_obj = [
          { id: 24, user_id: 1, schedule_type: 2, group_year: 2018, group_month: 8, date_key: dateHelper.createDate(2018, 8, 1).toDate(), time_from: "11:00", time_to: "17:00", shot_type: 1, event_name: "テストイベント", event_url: "htt://c2link.com/detail/event/test", cost: 1000, num: 1, cos_chara: null, remarks: "カメラマンテストスケジュール" },
          [ { schedule_id: 24, tag_id: 1 }, { schedule_id: 24, tag_id: 2 } ],
          [ { schedule_id: 24, prefecture_id: 13 } ],
        ]
        stub[0].withArgs(test_user_id, test_date_key).returns(stub_return_obj[0]);
        stub[1].withArgs(test_schedule_id).returns(stub_return_obj[1]);
        stub[2].withArgs(test_schedule_id).returns(stub_return_obj[2]);
