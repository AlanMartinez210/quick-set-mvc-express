  ...
  constructor(){
    ...
    for(var i=1;i<=6;i++){
      const d = {
        ...
        date_key: dateHelper.createDate(now_date.getFullYear(), now_date.getMonth()+1, (1+i))
      }
    ...
    for(var i=1;i<=5;i++){
      const d = {
        ...
        remarks: "カメラマンテストスケジュール",
        event_name: "テストイベント" + i,
        date_key: dateHelper.createDate(now_date.getFullYear(), now_date.getMonth(), (1+i))
      }
  ...
  genMatchingData(){
    return new Promise((resolve, reject) => {
      ...
      .then(res => {
        ...
        // スケジュールデータ(未来)を作成
        return db.Schedule.bulkCreate(this.schedule_future);
      })
      .then(res => {
        // スケジュールデータ(過去)を作成
        return db.Schedule.bulkCreate(this.schedule_past);
      })
      .then(res => {
        // マッチングデータを生成
        return db.Matching.bulkCreate(this.matching_data);
      })
      .then(res => {
        // 発言を追加
        return db.Chat.create(this.chat_data[0])
      })
      .then(res => {
        // 発言を既読に更新
        return Promise.all([
          db.Chat.update({ read_flag: true }, { where: { matching_id: 2, seq_id: 1 } }),
        ])
      })
      .then(res => {
        // 発言を追加
        return db.Chat.create(this.chat_data[1])
      })
      .then(res => {
        // 発言を既読に更新
        return Promise.all([
          db.Chat.update({ read_flag: true }, { where: { matching_id: 2, seq_id: 2 } }),
        ])
      })
      .then(res => {
        // 発言を追加
        return db.Chat.create(this.chat_data[2])
      })
      .then(res => {
